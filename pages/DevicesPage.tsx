  ...
  return (
    <div className="w-full max-w-md mx-auto h-full flex flex-col relative pb-32 no-scrollbar overflow-y-auto px-4">
      <div className="flex flex-col items-center justify-center py-8">
        <h2 className={`${isDarkMode ? 'text-white' : 'text-[#1D3C2B]'} text-3xl font-black`}>أجهزة التتبع</h2>
      </div>
      <div className="flex flex-col gap-4">
        {devices.map((device) => (
          <div key={device.id} className="p-4 rounded-2xl bg-white/5 border border-white/10 flex justify-between items-center">
            <div><h4 className="text-white font-black text-sm">{device.name}</h4><span className="text-white/40 text-[9px]">{device.id}</span></div>
            <button onClick={() => handleDelete(device.id)} className="text-red-500 text-xs font-bold">حذف</button>
          </div>
        ))}
      </div>
