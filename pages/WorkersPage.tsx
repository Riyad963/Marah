  ...
  return (
    <div className="w-full max-w-md mx-auto h-full flex flex-col relative pb-32 no-scrollbar overflow-y-auto px-4">
      <div className="flex flex-col items-center justify-center py-8">
        <h2 className={`${isDarkMode ? 'text-white' : 'text-[#1D3C2B]'} text-3xl font-black`}>فريق العمل</h2>
      </div>
        ...
        {workers.map((worker) => (
          <div key={worker.id} className="p-4 rounded-[1.5rem] border bg-white/5 border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div><h4 className="text-white font-black text-sm">{worker.name}</h4><p className="text-white/50 text-[10px]">{worker.role}</p></div>
            </div>
            <button onClick={() => handleDelete(worker.id)} className="text-red-400 text-xs">حذف</button>
          </div>
        ))}
      </div>
